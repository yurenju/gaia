...
function setWallpaper(callback) {
  ...
  if (!wallpaper.exists()) {
    wallpaper = utils.resolve(
      utils.joinPath('build', 'data', 'wallpaper' + devpixels + '.jpg'),
      config.GAIA_DIR);
  }

  if (!wallpaper.exists()) {
    wallpaper = utils.resolve(utils.joinPath('build', 'data', 'wallpaper.jpg'),
      config.GAIA_DIR);
  }
...
function execute(options) {
  ...
  var settingsFile = utils.getFile(config.GAIA_DIR, 'build', 'data',
