...
# Generate profile/
$(PROFILE_FOLDER): preferences app-makefiles copy-build-stage-manifest test-agent-config offline contacts extensions $(XULRUNNER_BASE_DIRECTORY) .git/hooks/pre-commit $(PROFILE_FOLDER)/settings.json create-default-data $(PROFILE_FOLDER)/installed-extensions.json
ifeq ($(BUILD_APP_NAME),*)
...
app-makefiles: $(BUILD_STAGE_APPS) svoperapps webapp-manifests $(XULRUNNER_BASE_DIRECTORY)
...
webapp-zip: webapp-optimize webapp-shared app-makefiles $(XULRUNNER_BASE_DIRECTORY)
ifneq ($(DEBUG),1)
...
webapp-shared: app-makefiles $(XULRUNNER_BASE_DIRECTORY)
ifneq ($(DEBUG),1)
...
$(PROFILE_FOLDER)/settings.json: profile-dir $(XULRUNNER_BASE_DIRECTORY)
ifeq ($(BUILD_APP_NAME),*)
