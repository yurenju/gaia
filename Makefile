...
ifeq "$(NODE_MODULES_SRC)" "modules.tar"
	-$(DOWNLOAD_CMD) https://github.com/yurenju/gaia-node-modules/tarball/$(NODE_MODULES_REV) &&\
...
ifeq "$(NODE_MODULES_SRC)" "modules.tar"
	$(TAR_WILDCARDS) --strip-components 1 -x -m -f $(NODE_MODULES_SRC) "yurenju-gaia-node-modules-*/node_modules"
else
	rm -fr node_modules
